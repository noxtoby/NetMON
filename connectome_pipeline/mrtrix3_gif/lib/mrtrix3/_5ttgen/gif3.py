def initialise(base_parser, subparsers):
  ...
  parser = subparsers.add_parser('gif3', author='Neil P. Oxtoby (noxtoby@gmail.com), via freesurfer.py by Robert E. Smith (robert.smith@florey.edu.au)', synopsis='Generate the 5TT image based on a GIF3 parcellation image', parents=[base_parser])
  parser.add_argument('input',  help='The input GIF3 (Neuromorphometrics) parcellation image (usually containing \'NeuroMorph_Parcellation\' in its name)')
  parser.add_argument('output', help='The output 5TT image')
  options = parser.add_argument_group('Options specific to the \'gif3\' algorithm')
  options.add_argument('-lut', help='Manually provide path to the lookup table on which the input parcellation image is based (e.g. GIF3ColourLUT.txt)')
...
def execute():
  ...
  if not os.path.exists('LUT.txt'):
    ...
    if not gif3_home:
      app.error('Environment variable GIFDB_HOME is not set. Please set this variable manually, or provide script with path to file GIF3ColourLUT.txt using -lut option.')
    lut_input_path = os.path.join(gif_home, 'GIF3ColourLUT.txt')
    if not os.path.isfile(lut_input_path):
      app.error('Could not find GIF3 lookup table file (expected location: ' + lut_input_path + '), and none provided using -lut')
  ...
  if not os.path.isfile(lut_output_path):
    app.error('Could not find lookup table file for converting GIF3 parcellation output to tissues (expected location: ' + lut_output_path + ')')
