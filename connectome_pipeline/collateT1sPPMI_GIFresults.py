...
I ended up running the steps in this manually within a python command line.
...
Option 2 (frozen/stable version of GIF, courtesy of Arman Eshaghi):
  cluster:/cluster/project0/MS_LATA/GIF_old/bin/seg_GIF
	...
	Usage:
		collateT1sPPMI.py PPMIDownloadFolder 
...
def traversePPMIFolders(topFolder='/home/noxtoby/pond/PPMI/T1-Anatomical'):
    """Traverses the file hierarchy within topFolder looking for files named 
    PPMI*MR_T1-anatomical*NeuroMorph_Parcellation.nii.gz 
    and returns a list containing the paths, including filename, for each file."""
    matches = []
    for root, dirnames, filenames in os.walk(topFolder):
        for filename in fnmatch.filter(filenames, 'PPMI*MR_T1-anatomical*NeuroMorph_Parcellation.nii.gz'):
...
def main():
    ...
    topFolder = workingDir + '/T1-Anatomical'
    structuralMRI = traversePPMIFolders(topFolder)
    
    filenameList = '%s/PPMI_T1Anatomical_GIFresults_files.txt' % (workingDir)
    ...
    folderList = '%s/PPMI_T1Anatomical_GIFresults_foldersToDelete.txt' % (workingDir)
